...
def get_two_dis_noise(pos_start, pos_end, vel_start, vel_end, theta_start, theta_end, dot_start, dot_end ,  points, HDC, LDC, steps):
    sigma = 0.1
    HDC_model = HDC
        ...
        for j in range(num_velocity - 1):
            ...
            for k in range(num_point):
                ...
                for m in range(steps):  # simulation for the LDC
                    ...
                    current_state = np.reshape(env.state + noise, (-1, 4))
...
points = 60; steps = 20
traj_CP, act_CP, max_diff_traj, max_diff_action = get_two_dis_noise(pos_start, pos_end, vel_start, vel_end, theta_start, theta_end, dot_start, dot_end ,  points, HDC_model, LDC_whole, steps)
