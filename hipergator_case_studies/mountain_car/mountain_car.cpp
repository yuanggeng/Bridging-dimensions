...
int main(int argc, char *argv[])
{
	...
	string net_name = "LDC_(-0.6, -0.4)-(-0.01, 0.05).txt";//argv[2]"";
...
*/
	...
	// pos: -0.6, -0.5; velocity: 0.01, 0.05
	...
	Interval CP(-0.02, 0.02);
	nn = NeuralNetwork(net_name);
	...
	if(b)
	{
		...
		ofstream reach_output("./outputs/reach_verification/" + good_result + "_" + to_string(start_pos) + to_string(start_velocity) +".txt");
		reach_result = "Verification result: Yes(" + to_string(steps) + ")";
	}
	else
	{
	...
	plot_setting.plot_2D_octagon_GNUPLOT("./outputs/", benchmark_name + "_" + to_string(if_symbo) + "_" + to_string(start_pos) + "_" + to_string(start_velocity), result.tmv_flowpipes, setting);
