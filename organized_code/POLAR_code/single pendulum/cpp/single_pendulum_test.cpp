...
int main(int argc, char *argv[])
{
	...
	// string net_name1 = "new_LDC1_theta0-0.5_dot-2-0.txt";
	// string net_name2 = "new_LDC2_theta0.5-1_dot-2-0.txt";
	// string net_name3 = "new_LDC3_theta1-1.5_dot-2-0.txt";
	// string net_name4 = "new_LDC4_theta1.5-2_dot-2-0.txt";


	string net_name2 = "new_LDC2_theta0.5-1_dot-2-0.txt";
	string net_name3 = "LDC3noise_theta1-1.5_dot-2-0.txt";
	string net_name4 = "LDC4noise_theta1.5-2_dot-2-0.txt";
...
// the initial region: [0,2]x[-2,0] to save time [0.25,1.85]x[-2,0]
	for (double start = 0.6; start <= 1.8; start += 0.01){
	int steps = 30;
	...
	for (double startx1 = -2; startx1 <= 0; startx1 += 0.01){
	...
	if (1 <= startpoint && startpoint+0.01 <= 1.5){
		nn_name = net_name3;
		nn = NeuralNetwork(nn_name);
		Interval CP(-0.09, 0.09);
		...
		nn = NeuralNetwork(nn_name);
		Interval CP(-0.09, 0.09);
	}else if (0.8 <= startpoint && startpoint + 0.01 <= 1){
		nn_name = net_name2;
		nn = NeuralNetwork(nn_name);
		Interval CP(-0.09, 0.09);
	...
	// const std::string filetable = "test_HSCC.txt";
	const std::string filetable = "test_HSCC.txt";
    std::vector<std::vector<double>> matrix = readMatrixFromFile(filetable);
	
	std::cout << "the matrix size is" << matrix[0].size() << endl;
	//std::cout << "One element is" << matrix[4][4] << endl;

	std::vector<int> indices;
    for (int i = 0; i < matrix[0].size(); ++i) {
        if (matrix[0][i] >= 1 && matrix[0][i] <= 10 && matrix[1][i] >= 20 && matrix[1][i] <= 30) {
            indices.push_back(i);
        }
    }
	for (int elem : indices){

		std::cout << "all the indices display:" << elem << endl;
	}
	//std::cout << "all the indices display:" << indices << endl;
    ...
    // 	std::cout << "Rounded up to the nearest intematrixger/ column number: " << rounded_column_initial << std::endl;
