...
int main(int argc, char *argv[])
{
	...
	string net_name1 = "POLAR_LDC1_theta0-6.28_dot0-8_wholestate.txt";
	string net_name2 = "POLAR_LDC2_theta0-6.28_dot0--2.txt";
	string net_name3 = "POLAR_LDC3_theta0-6.28_dot-2-4.txt";
	...
	for (double start = 0; start <= 2; start += 0.01) //0,2
	{
		int steps = 30;
		// float startpoint = 1.3;
		float startpoint = floorf(start * 100) / 100;
		double endpoint = startpoint + 0.01;
		for (double startx1 = -2; startx1 <= 0; startx1 += 0.01) //-2,0
		{
			...
			if (0 <= startpoint && startpoint + 0.01 <= M_PI * 2 && startx1 + 0.01 <= 0 && -2 <= startx1)
			{
				string nn_name = net_name2;
				nn = NeuralNetwork(nn_name);
				// CP = Interval(-0.017, 0.0170);
			}
			else if (0 <= startpoint && startpoint + 0.01 <= M_PI * 2 && startx1 + 0.01 <= -2 && -4 <= startx1)
			{
			...
			for (int iter = 0; iter < steps; ++iter)
			{
				...
				*/
				...
				if (0 <= end_box[0].inf() && end_box[0].sup() <= M_PI * 2 && end_box[1].sup() <= 0 && -2 <= end_box[1].inf())
				{
					nn_name = net_name2;
					nn = NeuralNetwork(nn_name);
					// CP = Interval(-0.017, 0.017);
				}
				else if (0 < end_box[0].inf() && end_box[0].sup() < M_PI * 2 && end_box[1].sup() <= -2 && -4 <= end_box[1].inf())
				{
					nn_name = net_name3;
					nn = NeuralNetwork(nn_name);
					// CP = Interval(-0.021, 0.021);
