...
int main(int argc, char *argv[])
{
	...
	string net_name = "POLAR_LDC1_theta0-6.28_dot0-8_wholestate.txt";
	string net_name1 = "noise0.00003_LDC_table/0.00003theta_0.00003thetadot_LDC1_theta0-0.5_dot-2-0.txt";
	string net_name2 = "noise0.00003_LDC_table/0.00003theta_0.00003thetadot_LDC2_theta0.5-1_dot-2-0.txt";
	string net_name3 = "noise0.00003_LDC_table/0.00003theta_0.00003thetadot_LDC3_theta1-1.5_dot-2-0.txt";
	string net_name4 = "noise0.00003_LDC_table/0.00003theta_0.00003thetadot_LDC3_theta1.5-2_dot-2-0.txt";

	string net_discrepancy = "POLAR_LDC1_theta0-6.28_dot0-8_wholestate.txt";
...
//	intervalNumPrecision = 600;
  ...
  //	setting.prepare();
	...
	vector<Constraint> targetSet;
	    ...
	    Constraint c3("x0 - 0.35", vars);           //x0 >= 0
	    Constraint c4("-x0 + 0", vars);
...
// the initial region: [0,2]x[-2,0] to save time [0.25,1.85]x[-2,0]
	clock_t begin, end;
	begin = clock();
	...
	for (double start = start_x0; start <= end_x0; start += 0.01){
	int steps = 30;
	//float startpoint = 1.3;
	
	//float startpoint = floorf(start * 100) / 100;
	double startpoint = start;
	startpoint = std::round(startpoint *100)/100;


	double  endpoint = startpoint + 0.01;
	for (double startx1 = start_x1; startx1 <= end_x1; startx1 += 0.01){
	...
	// define the neural network controller in different initial set
	NeuralNetwork nn; //declare the nn outside of the if and else
	string nn_name;
	if (0 <= startpoint && startpoint + 0.01 <= 0.5){
		nn_name = net_name1;
		nn = NeuralNetwork(nn_name);

	}else if (0.5 <= startpoint && startpoint + 0.01 <= 1){
		nn_name = net_name2;
		nn = NeuralNetwork(nn_name);

	}else if (1 <= startpoint && startpoint+0.01 <= 1.5){
		nn_name = net_name3;
		nn = NeuralNetwork(nn_name);

	}else if (1.5 <= startpoint && startpoint + 0.01 <= 2){
		nn_name = net_name4;
		nn = NeuralNetwork(nn_name);
	}

	std::cout << "the startpoint X0 " << startpoint<< std::endl;
	std::cout << "the end point X0+0.01  " << startpoint + 0.01 << std::endl;


	std::cout << "neural network that will be used:" << nn_name << endl;
		const std::string filename = "new_all_CP_table.txt";
        std::vector<std::vector<double>> matrix = readMatrixFromFile(filename);
	// Find the exact action CP value in the matrix based on initial set
		int rounded_row_initial = static_cast<int>(std::ceil(endpoint * 10));
    	std::cout << "the endpoint of x0 " << endpoint<< std::endl;
    	std::cout << "Rounded up to the nearest integer/ row number: " << rounded_row_initial << std::endl;

		int rounded_column_initial = static_cast<int>(std::ceil((startx1 + 2 + 0.01)*10));

    	std::cout << "Original value for X1: " << startx1 + 0.01 << std::endl;
	...
	//Print the CP value in the table
		...
		if (rounded_row_initial - 1 > 20){
			rounded_row_initial = rounded_row_initial - 1;
		}
		if (rounded_column_initial - 1 > 20){
			rounded_column_initial = rounded_column_initial - 1;
		}
		double cp_value = matrix[rounded_row_initial - 1 ][rounded_column_initial - 1];
		Interval CP(-cp_value, cp_value);

	// Previous version: Find the exact CP value in the matrix
	/*
	...
	NeuralNetwork nn_reg; //load the discrepancy regression model.
	nn_reg = NeuralNetwork(net_discrepancy);
 ...
 /////////////////////////////////////////////////////////////////////////////////////
	for (int iter = 0; iter < steps; ++iter)
	{
		...
		TaylorModelVec<Real> tmv_output_reg;
		...
		//get the output of the regression model
		Interval box_reg;
		nn_reg.get_output_tmv_symbolic(tmv_output_reg, tmv_input, initial_set.domain, polar_setting, setting);
		tmv_output_reg.tms[0].intEval(box_reg, initial_set.domain);
        cout << "discrepancy output range: " << box_reg << endl;
